...
let appleServerLibrary;
let verifiers;

function getAppleServerLibrary() {
  appleServerLibrary ??= require("@apple/app-store-server-library");
  return appleServerLibrary;
}

function createVerifier(environment) {
  const {Environment, SignedDataVerifier} = getAppleServerLibrary();
...
function getVerifiers() {
  ...
  const {Environment} = getAppleServerLibrary();
  verifiers = {
...
async function verifyAppleTransaction(signedTransaction) {
  ...
  let lastError;
  for (const [environment, verifier] of Object.entries(getVerifiers())) {
    try {
